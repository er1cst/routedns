...
func instantiateRouter(id string, r router, resolvers map[string]rdns.Resolver) error {
	...
	for _, route := range r.Routes {
		...
		r, err := rdns.NewRoute(route.Name, route.NameFile, route.Class, types, route.Weekdays, route.Before, route.After, route.Source, route.DoHPath, route.Listener, route.TLSServerName, resolver)
		if err != nil {
